...
const disableCharacters = (option, key) => {
  const discartedCharacters = guessCharacter[key] === option ? persons.filter((person) => person[key] !== option) : persons.filter((person) => person[key] !== option);
  // console.log(discartedCharacters);
